...
%Ao analisar a Tabela \ref{tb:nemenyi}, nota-se uma diferença significativa entre os modelos. Por exemplo, os valores na interseção da linha DT (G) e coluna GRU (H) são $0$, o que indica uma diferença particularmente crítica entre esses modelos. Da mesma forma, os valores na interseção da linha DT (G) e coluna RNN (O) e coluna LSTM (I) também são muito baixos, evidenciando outra diferença de importância crítica entre esses modelos. Essas disparidades ressaltam a necessidade de uma análise minuciosa das diferenças estatísticas entre os modelos para determinar qual deles demonstra o desempenho mais apropriado para os propósitos da análise.
%
%Ao analisar a Tabela \ref{tb:nemenyi1}, observa-se que há diferenças significativas entre os modelos. Por exemplo, os valores na interseção da linha DT (G) e coluna GRU (H) são $0,012$, indicando uma diferença estatisticamente significativa entre esses modelos. Da mesma forma, os valores na interseção da linha DT (G) e coluna RNN (O) também são baixos, sugerindo uma diferença estatisticamente significativa entre esses modelos. Esses achados ressaltam a importância de examinar minuciosamente as diferenças estatísticas entre os modelos para determinar qual deles oferece o desempenho mais adequado para os propósitos da análise.
%
%
%A análise a Tabela \ref{tb:nemenyi2} revela diferenças significativas entre os modelos. Por exemplo, os valores na interseção da linha CNN (F) e coluna DT (G) são $0,023$, indicando uma diferença estatisticamente significativa entre esses modelos. Da mesma forma, os valores na interseção da linha DT (G) e coluna GRU (H) também são baixos, sugerindo outra diferença estatisticamente significativa. Esses resultados enfatizam a importância de uma avaliação criteriosa das diferenças estatísticas entre os modelos para determinar qual deles oferece o desempenho mais adequado para os propósitos da análise.
